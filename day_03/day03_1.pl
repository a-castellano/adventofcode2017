...
my $octet_where_number_is_in = ceil( ( $number - $numbers_in_previous_dimension ) / $numbers_by_octet );

print "$number is placed in $octet_where_number_is_in th octet\n";

my $previous_octet_value = ( $octet_where_number_is_in - 1) * $numbers_by_octet +  $numbers_in_previous_dimension;

print "Previous octet value -> $previous_octet_value\n";

my $numbers_from_previous_octet_value_to_number = $number - $previous_octet_value;

print "Numbers from previous octet value to number -> $numbers_from_previous_octet_value_to_number\n";

my $steps;

if ( $octet_where_number_is_in % 2)
{
  $steps = $max_steps_in_this_dimension - $numbers_from_previous_octet_value_to_number;
}
else
{
  $steps = $min_steps_in_this_dimension + $numbers_from_previous_octet_value_to_number;
}
