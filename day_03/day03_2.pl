...
sub get_next_value {
    ...
    if ( $edge_index >= 0 ) {
        ...
        push( @added_positions, $candidate_position )
          ;    #unless ( $candidate_position ~~ @added_positions );
...
# position is just next edge, add 2 previous dimiension positions and position behind edge
        if ( $edge_index >= 0 ) {
            ...
            my $previous_edge_index =
              get_next_edge_index( \@current_edges, $next_position - 2 );
            $candidate_position =
              $next_position -
              $numbers_in_previous_dimension -
              2 * ( $previous_edge_index + 1 );
            push( @added_positions, $candidate_position )
              ;    #unless ( $candidate_position ~~ @added_positions );
            $candidate_position--;
            ...
            if ( $edge_index >= 0 ) {
                my $next_edge_index =
                  get_next_edge_index( \@current_edges, $next_position );
                my $candidate_position =
                  $next_position -
                  $numbers_in_previous_dimension - 1 -
                  2 * $next_edge_index;
                ...
                push( @added_positions, $candidate_position )
                  ;    #unless ( $candidate_position ~~ @added_positions );
                if ( $next_edge_index ==
                    3 )    #This position is viewing last spiral start
                {
                ...
                if ( $edge_index == -1
                    && get_index( \@previous_edges, $current_position ) == 3 )

                  #first position of dimension
                {
                    my $candidate_position =
                      $next_position - $numbers_in_previous_dimension;
                    push( @added_positions, $candidate_position )
                      ;    #unless ( $candidate_position ~~ @added_positions );
                }
                else {
                    if ( $edge_index == -1
                        && get_index( \@previous_edges, $current_position - 1 )
                        == 3 )
...
#position is no special and has 3 adyacent positions in preivous position spiral
                    {
                        my $next_edge_index =
                          get_next_edge_index( \@current_edges,
                            $next_position );
                        my $candidate_position =
                          $next_position -
                          $numbers_in_previous_dimension -
...
while ( $current_value <= $number_to_find ) {
    $value =
      get_next_value( \@array, \@current_edges, \@previous_edges,
        $current_position, $numbers_in_previous_dimension,
      );
    push( @array, $value );
    $current_position++;
    ...
    if ( $current_position >= $max_position_in_current_dimension ) {
        ...
        $max_position_in_previous_dimension =
          $max_position_in_current_dimension;
        $max_position_in_current_dimension =
          get_max_number_from_dimensiom($current_dimension);
        ...
        for ( my $i = 0 ; $i < 4 ; $i++ ) {
            push @current_edges,
              ( $max_position_in_current_dimension - $edge_lenght * $i );
        }
