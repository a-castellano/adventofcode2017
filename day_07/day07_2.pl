...
while ( my $line = <$fh> ) {
    ...
    if ( $line =~ m/->/ )
      ...
      # program has child(s)
    {
        my ( $program_name, $value, $childs ) =
          $line =~ /(.*)? \((\d+)\) -> (.+)$/;
        $programs{$program_name} =
          { create_node( $program_name, $value, 1, $childs ) };
    }
    else {
...
if ( $value[0] ) {
    if ( $value[2] > $value[3] ) {
        $right_value =
          $programs{ $value[1] }{'value'} + ( $value[2] - $value[3] );
    }
    else {
        $right_value =
          $programs{ $value[1] }{'value'} - ( $value[3] - $value[2] );
    }

    print "There is an unbalanced node in this tree.\n";
    print
"Value of node $value[1] should be $right_value instead of $programs{$value[1]}{'value'}\n";
}
