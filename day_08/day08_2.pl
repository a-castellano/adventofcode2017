...
while ( my $line = <$fh> ) {
    ...
    my ( $variable_to_modify, $operator, $addend, $variable_to_compare,
        $logic_operator, $value_to_compare )
      = $line =~
    ...
    if (
        evaluate_logical_expression(
            $variables{$variable_to_compare}, $logic_operator,
            $value_to_compare
        )
      )
    {
    ...
    $largest_value_registered = $variables{$variable_to_modify}
      if ( $variables{$variable_to_modify} > $largest_value_registered );
}
