...
sub calculate_next_direction{
  ...
  my $turn_to = $_[1];
    ...
    if ( $direction eq "Up" ) {
        ...
        elsif ( $turn_to eq "Left" ) {
            $direction = "Left";
        }
        elsif ( $turn_to eq "Reverse" ) {
            $direction = "Down";
        }
    }
...
sub make_burst {
    ...
    if ( $grid->[ $current_position->[0] ][ $current_position->[1] ] eq '.' ) {
        $grid->[ $current_position->[0] ][ $current_position->[1] ] = 'W';
        $turn_to = "Left";
    }
    elsif ( $grid->[ $current_position->[0] ][ $current_position->[1] ] eq 'W' )
    {
        $grid->[ $current_position->[0] ][ $current_position->[1] ] = '#';
        $infected_node = 1;
    }
    elsif ( $grid->[ $current_position->[0] ][ $current_position->[1] ] eq '#' )
    {
        $grid->[ $current_position->[0] ][ $current_position->[1] ] = 'F';
        $turn_to = "Right";
    }
    elsif ( $grid->[ $current_position->[0] ][ $current_position->[1] ] eq 'F' )
    {
        $grid->[ $current_position->[0] ][ $current_position->[1] ] = '.';
        $turn_to = "Reverse";
    }
    ...
    $direction = calculate_next_direction($direction, $turn_to);
